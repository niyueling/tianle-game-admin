...
class OperationLog
{
    ...
    public function handle($response)
    {
        ...
        //不记录日志操作
        if (strtolower(str_replace('.', '\\', $request->controller())) === 'settings\system\log') {
            return;
        }
        ...
        $systemLog->admin_name = $request->adminInfo['username'] ?? '';
        $systemLog->action = $notes;
        $systemLog->account = $request->adminInfo['username'] ?? '';
