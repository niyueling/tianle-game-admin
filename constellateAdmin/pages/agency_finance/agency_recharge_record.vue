					...
					<view class="order" v-for="item in lists" :key="item._id">
						<view class="order-header">
							<view class="flex user">
								<view>用户: {{ item.agency.username }}</view>
								<view>({{ item.agency.role }})</view>
							</view>
							<view class="order-status">
								<view class="order-status--primary">{{ item.currency_str }}</view>
							</view>
						</view>
							...
							<view class="goods-wrap">
								<view class="m-l-16 line-1">
									<view class="muted flex row-between xs m-t-20">
										<view style="color: red;">房卡：{{ item.amount }} 张</view>
										<view>类型:{{item.source_str}}</view>
									</view>
									<view class="muted flex row-between xs m-t-20">
										<view>操作类型:{{ item.action_str }}</view>
