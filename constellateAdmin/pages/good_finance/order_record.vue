					...
					<view class="order" v-for="item in lists" :key="item._id">
						<view class="order-header">
							<view class="flex user">
								<view>{{ item.playerInfo.name }}</view>
								<view>({{ item.playerInfo.shortId }})</view>
								<view>({{ item.playerInfo.realName }})</view>
							</view>
							<view class="order-status">
								<view class="order-status--primary">{{ item.status }}</view>
							</view>
						</view>
							...
							<view class="goods-wrap">
								<view class="m-l-16 line-1">
									<view class="muted flex row-between xs m-t-20">
										<view>数量:{{ item.goodsDetail.amount }}</view>
										<view>类型:{{item.goodsDetail.goodsType}}</view>
									</view>
									<view class="muted flex row-between xs m-t-20">
										<view>安卓价:{{ item.goodsDetail.androidPrice }}</view>
										<view>苹果价:{{item.goodsDetail.iosPrice }}</view>
									</view>
									<view class="muted flex row-between xs m-t-20">
										<view style="color: red;">实付:{{ item.payPrice }}</view>
