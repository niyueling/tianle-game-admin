...
<template>
    <div class="user">
            ...
            <div class="card-content m-t-24">
                <el-row :gutter="20">
                    <el-col :span="6" class="flex-col col-center">
                        <div class="lighter m-b-8">累计钻石</div>
                        <div class="font-size-30">{{ summary.totalGem }}</div>
                    </el-col>
                    <el-col :span="6" class="flex-col col-center">
                        <div class="lighter m-b-8">累计红包</div>
                        <div class="font-size-30">{{ summary.totalRedPacket }}</div>
                    </el-col>
                    <el-col :span="6" class="flex-col col-center">
                        <div class="lighter m-b-8">累计消耗</div>
                        <div class="font-size-30">{{ summary.totalConsume }}</div>
                    </el-col>
                </el-row>
            </div>
        </div>
        ...
        <div class="ls-card m-t-24 ls-chart--visitors">
            ...
            <div class="content">
                <e-chart class="chart" :option="consume" />
            </div>
        </div>
    </div>
</template>
...
import { apiConsumeAnalysis } from '@/api/data/data'
echarts.use([
    BarChart,
    GridComponent,
    CanvasRenderer,
...
export default class User extends Vue {
    ...
    getDataCenterVisit() {
        // 清空echarts 数据
        this.consume.xAxis.data = []

        apiConsumeAnalysis(this.form).then(res => {
            this.summary = res.summary
            this.consume.xAxis.data = res.consume.date

            res.consume.list.forEach((item: any, index: number) => {
                this.consume.series[index].data = item.data
                this.consume.series[index].name = item.name
                this.consume.legend.data[index] = item.name
            })
        })
    }
