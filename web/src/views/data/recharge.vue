        ...
        <div class="ls-card ls-card-top">
            <div class="journal-search m-t-16">
                <el-form ref="formRef" inline :model="summary" label-width="80px" size="small">
                    ...
                    <el-button size="small" type="text" @click="onReset">重置</el-button>
                </el-form>
            </div>
        </div>
            ...
            <div class="card-content m-t-24">
                <el-row :gutter="20">
                    <el-col :span="6" class="flex-col col-center">
                        <div class="lighter m-b-8">累计充值</div>
                        <div class="font-size-30">{{ summary.total }}</div>
                    </el-col>
                    <el-col :span="6" class="flex-col col-center">
                        <div class="lighter m-b-8">微信充值</div>
                        <div class="font-size-30">{{ summary.wechat }}</div>
                    </el-col>
                    <el-col :span="6" class="flex-col col-center">
                        <div class="lighter m-b-8">后台充值</div>
                        <div class="font-size-30">{{ summary.admin }}</div>
                    </el-col>
                </el-row>
            </div>
        </div>
...
import { apiRechargeAnalysis } from '@/api/data/data'
echarts.use([
    BarChart,
    GridComponent,
    CanvasRenderer,
...
export default class User extends Vue {
    ...
    getDataCenterVisit() {
        // 清空echarts 数据
        this.recharge.xAxis.data = []

        apiRechargeAnalysis(this.form).then(res => {
            this.summary = res.summary
            this.recharge.xAxis.data = res.recharge.date

            res.recharge.list.forEach((item: any, index: number) => {
