<template>
    <div class="user-withdrawal">
        <div class="ls-card">
            <el-alert class="xxl" title="4王以上对局记录。" type="info" :closable="false" show-icon> </el-alert>
            <div class="journal-search m-t-16">
                <el-form ref="formRef" inline :model="form" label-width="70px" size="small" class="ls-form">
                    ...
                    <el-form-item label="王数">
                        <el-input class="ls-select-keyword" v-model="form.jokerNum" placeholder="请输入王数"></el-input>
                    </el-form-item>
                    <el-form-item label="房间号">
                        <el-input class="ls-select-keyword" v-model="form.roomId" placeholder="请输入房间号"></el-input>
                    </el-form-item>
                    <el-form-item label="日期">
                        <el-input class="ls-select-keyword" v-model="form.dayId" placeholder="请输入日期"></el-input>
                    </el-form-item>
            ...
            <div class="list-table m-t-16">
                ...
                >
                    ...
                    <el-table-column prop="roomId" label="房间号">
                        <template slot-scope="scope">
                            <div @click="navicatToRoomById(scope.row.roomId)" style="color: #3967ff; cursor: pointer">
                                {{ scope.row.roomId }}
                            </div>
                        </template>
                    </el-table-column>
                    <el-table-column prop="type" label="游戏"> </el-table-column>
                    <el-table-column prop="juShu" label="小局"></el-table-column>
                    <el-table-column prop="juCount" label="局数"> </el-table-column>
                    <el-table-column prop="jokerNum" label="王数"> </el-table-column>
                    <el-table-column prop="jokerCount" label="王总数"> </el-table-column>
                    <el-table-column prop="todayJuCount" label="今日局数"> </el-table-column>
                    <el-table-column prop="dayId" label="游戏日期"> </el-table-column>
...
import { apiClubCreatorGoldLog, apiFourJokerGameLog } from '@/api/finance/finance'
...
export default class AccountLog extends Vue {
    ...
    DetailsClick(item: any) {
        localStorage.setItem('cards', JSON.stringify(item.cards))
        this.$router.push({
    ...
    formatInitTime() {
        ...
        // 获取昨日零点时间并转化为格式化字符串
        let yesterdayZeroTime = new Date(
            today.getFullYear(),
            today.getMonth(),
            today.getDate() - 1,
            0,
        ...
        // 获取昨日23：59：59时间并转化为格式化字符串
        let yesterdayEndTime = new Date(
            today.getFullYear(),
            today.getMonth(),
            today.getDate() - 1,
            23,
        ...
        // 输出格式化后的时间
        console.log('昨日零点时间：' + yesterdayZeroTime) // 2022-03-08 00:00:00
        console.log('昨日23：59：59时间：' + yesterdayEndTime) // 2022-03-08 23:59:59
        return [yesterdayZeroTime, yesterdayEndTime]
    }
    ...
    onExport() {
        this.form.export = 1
        apiFourJokerGameLog(this.form).then((res: any) => {
