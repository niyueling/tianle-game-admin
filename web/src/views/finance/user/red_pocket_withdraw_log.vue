<template>
    <div class="user-withdrawal">
        <div class="ls-card">
            <el-alert class="xxl" title="温馨提示： 1.会员红包提现记录。" type="info" :closable="false" show-icon>
            </el-alert>
                    ...
                    <el-form-item label="状态">
                        <div class="flex">
                            <el-select style="width: 120px" v-model="form.state" placeholder="全部">
                                ...
                                <el-option label="成功" value="finished"></el-option>
                                <el-option label="失败" value="403"></el-option>
                            </el-select>
                        </div>
                    </el-form-item>
        ...
        <div class="ls-withdrawal__centent ls-card m-t-16">
            <div class="list-table m-t-16">
                ...
                >
                    <el-table-column prop="_id" label="日志编号"> </el-table-column>
                    <el-table-column prop="playerInfo.shortId" label="用户编号">
                        <template slot-scope="scope">
                            <div
                                @click="navicatToUserByShortId(scope.row.playerInfo.shortId)"
                                style="color: #3967ff; cursor: pointer"
                            >
                                {{ scope.row.playerInfo.shortId }}
                            </div>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column prop="playerInfo.phone" label="手机号码">
                        <template slot-scope="scope">
                            <div
                                @click="navicatToUserByPhone(scope.row.playerInfo.phone)"
                                style="color: #3967ff; cursor: pointer"
                            >
                                {{ scope.row.playerInfo.phone }}
                            </div>
                        </template>
                    </el-table-column>
                    <el-table-column prop="amountInFen" label="提现金额"></el-table-column>
                    <el-table-column prop="info" label="提现状态"> </el-table-column>
...
import { apiClubCreatorGoldLog, apiUserRedPocketithdrawLog } from '@/api/finance/finance'
...
export default class AccountLog extends Vue {
    ...
    getList(page?: number): void {
        ...
        this.pager
            .request({
                callback: apiUserRedPocketithdrawLog,
                params: {
                    ...this.form
                }
            })
    ...
    onExport() {
        this.form.export = 1
        apiUserRedPocketithdrawLog(this.form).then((res: any) => {
