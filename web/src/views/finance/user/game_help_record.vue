<template>
    <div class="user-withdrawal">
        <div class="ls-card">
            <el-alert class="xxl" title="温馨提示： 游戏待救助列表。" type="info" :closable="false" show-icon> </el-alert>
        ...
        <div class="ls-withdrawal__centent ls-card m-t-16">
            <div class="list-table m-t-16">
                ...
                >
                    ...
                    <el-table-column prop="estimateLevel" label="救助牌型">
                        <template slot-scope="scope">
                            <div v-if="scope.row.game==='zhadan'"> {{ scope.row.estimateLevel }}星 </div>
                            <div v-if="scope.row.game==='majiang'"> {{ scope.row.mahjongStr }} </div>
                            <div v-if="scope.row.game==='paodekuai'"> {{ scope.row.pdkStr }} </div>
                            <div v-if="scope.row.game==='shisanshui'"> {{ scope.row.sssStr }} </div>
                            <div v-if="scope.row.game==='biaofen'"> {{ scope.row.bfStr }} </div>
                        </template>
                    </el-table-column>
                    <el-table-column prop="count" label="救助次数"> </el-table-column>
                    <el-table-column prop="juCount" label="救助局数"> </el-table-column>
                    <el-table-column prop="typeStr" label="救助类型"> </el-table-column>
...
import { apiUserGameHelpList } from '@/api/finance/finance'
...
export default class AccountLog extends Vue {
    ...
    isFormEmpty() {
        ...
        this.form.start_time = this.time.start_time ? this.time.start_time : ''
        this.form.end_time = this.time.end_time ? this.time.end_time : ''
    }
