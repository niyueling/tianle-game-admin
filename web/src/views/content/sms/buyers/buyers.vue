        ...
        <div class="ls-card">
            <div class="m-t-24">
                <el-table :data="pager.lists" v-loading="pager.loading" style="width: 100%" size="mini">
                    ...
                    <el-table-column label="系统消息" min-width="180">
                        <template slot-scope="scope">
                            <!-- 系统消息的当前状态 -->
                            <el-tag :type="scope.row.system_notice.status == 0 ? 'danger' : 'success'" effect="plain">
                                {{ scope.row.system_notice.status == 0 ? '关闭' : '启用' }}
                            </el-tag>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column label="微信模板" min-width="180">
                        <template slot-scope="scope">
                            <!-- 微信模板的当前状态 -->
                            <el-tag :type="scope.row.oa_notice.status == 0 ? 'danger' : 'success'" effect="plain">
                                {{ scope.row.oa_notice.status == 0 ? '关闭' : '启用' }}
                            </el-tag>
                        </template>
                    </el-table-column>
                    <el-table-column prop="login_time" label="微信小程序" min-width="180">
                        <template slot-scope="scope">
                            <!-- 微信小程序的当前状态 -->
                            <el-tag :type="scope.row.mnp_notice.status == 0 ? 'danger' : 'success'" effect="plain">
                                {{ scope.row.mnp_notice.status == 0 ? '关闭' : '启用' }}
                            </el-tag>
                        </template>
                    </el-table-column>
...
import { apiNoticeData } from '@/api/content/sms'
